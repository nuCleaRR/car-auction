...
public static class DbInitializer
{
    public static void InitDb(WebApplication app)
    {
        ...
        SeeData(scope.ServiceProvider.GetRequiredService<AuctionDbContext>());
    }

    private static void SeeData(AuctionDbContext context)
    {
        context.Database.Migrate();

        if (context.Auctions.Any())
        {
            Console.WriteLine("No need to seed init data");
            return;
        }
